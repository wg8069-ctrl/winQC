...
async function generateAndSendExcel(data, wgNumber, reporterName, photoUrl, photoUrl2) {
  ...
  if (!fs.existsSync(templatePath)) {
    const msg = 'template.xlsx 不存在，路徑: ' + templatePath;
    console.error(msg);
    return { buffer: null, downloadUrl: null, error: msg };
  }
  ...
  try {
    ...
    console.log('Template loaded, sheet:', ws.name);
  } catch (e) {
    const msg = 'template.xlsx 讀取失敗: ' + e.message;
    console.error(msg);
    return { buffer: null, downloadUrl: null, error: msg };
  }

  // Step 3: 建立對照表 — {{佔位符}} 對應值
  const mapping = {
    '異常單號': wgNumber,
    ...
    '判定': data.judge || '',
    '回報人': reporterName || '',
    '人工成本人': data.laborPeople ? String(data.laborPeople) : '',
    '人工成本時': data.laborHours ? String(data.laborHours) : '',
    '行政成本人': data.adminPeople ? String(data.adminPeople) : '',
    '行政成本時': data.adminHours ? String(data.adminHours) : '',
    '所耗人力成本': data.laborCost || '',
    '異常標註內容': data.remark || '',
    '照片1': '',
    '照片2': '',
    // 以下是 M1~M7 區塊的欄位（從 Google Sheets 帶入）
    '外發加工': data.outsource || '',
    '試組': data.trialAssembly || '',
    '線前加工': data.preLine || '',
    '上線中': data.onLine || '',
    '特採': data.specialAccept || '',
    '退料': data.returnMat || '',
    '預計回廠時間': data.returnDate || '',
    '上線日期': data.onLineDate || '',
    '出貨日/待驗日': data.shipDate || '',
    '產線處理': data.lineHandle || '',
    '來廠處理': data.vendorHandle || '',
    '外廠處理': data.extHandle || '',
    '補料狀況': data.supplyStatus || '',
    '補料時間': data.supplyTime || '',
    '停線': data.stopLine || '',
    '無法一次完線': data.cantFinish || '',
    '再上線': data.reOnLine || '',
    '庫存數量': data.stockQty || '',
    '庫存處理': data.stockHandle || '',
    '加工': data.processing || '',
    '報廢': data.scrap || '',
  };

  // Step 4: 自動掃描 Template，替換所有 {{佔位符}}
  try {
    let filled = 0;
    ws.eachRow((row) => {
      row.eachCell((cell) => {
    ...
    console.log('Placeholders filled:', filled, 'for:', wgNumber);
  } catch (e) {
    const msg = '佔位符替換失敗: ' + e.message;
    console.error(msg);
    return { buffer: null, downloadUrl: null, error: msg };
  }
  ...
  for (const [url, col, row] of [[photoUrl, 0, 4], [photoUrl2, 6, 4]]) {
    if (url) {
      console.log('Fetching photo:', url.substring(0, 80));
  ...
  try {
    ...
    const msg = 'Excel writeBuffer 失敗: ' + e.message;
    console.error(msg);
    return { buffer: null, downloadUrl: null, error: msg };
  }

  // Step 7: 上傳 Cloudinary
  const filename = `${wgNumber}.xlsx`;
  const downloadUrl = await uploadExcelToCloudinary(buffer, filename);

  if (!downloadUrl) {
    return { buffer, downloadUrl: null, error: 'Cloudinary 上傳失敗，請檢查 API key/secret' };
  }
...
app.post('/api/anomaly', async (req, res) => {
  try {
    ...
    generateAndSendExcel(d, wgNumber, reporterName, photoUrl, photoUrl2)
      .then((result) => {
        if (result.downloadUrl) console.log('Excel uploaded:', result.downloadUrl);
        else console.error('Excel failed:', result.error);
      })
...
app.post('/api/generate-excel-from-sheet', async (req, res) => {
  try {
    ...
    const mapped = {
      ...
      // M1~M7 區塊
      outsource:    data['外發加工'] || '',
      trialAssembly:data['試組'] || '',
      preLine:      data['線前加工'] || '',
      onLine:       data['上線中'] || '',
      specialAccept:data['特採'] || '',
      returnMat:    data['退料'] || '',
      returnDate:   data['預計回廠時間'] || '',
      onLineDate:   data['上線日期'] || '',
      shipDate:     data['出貨日/待驗日'] || '',
      lineHandle:   data['產線處理'] || '',
      vendorHandle: data['來廠處理'] || '',
      extHandle:    data['外廠處理'] || '',
      supplyStatus: data['補料狀況'] || '',
      supplyTime:   data['補料時間'] || '',
      stopLine:     data['停線'] || '',
      cantFinish:   data['無法一次完線'] || '',
      reOnLine:     data['再上線'] || '',
      stockQty:     data['庫存數量'] || '',
      stockHandle:  data['庫存處理'] || '',
      processing:   data['加工'] || '',
      scrap:        data['報廢'] || '',
    };
    ...
    if (result.error) {
      ...
      res.status(500).json({ success: false, error: 'Cloudinary 上傳失敗（downloadUrl 為空）' });
    }
  } catch (err) {
