...
app.post('/api/anomaly', async (req, res) => {
  try {
    ...
    // LIFF 欄位對應到你現有的 Notion 欄位
    const notionData = {
      location:   d.unit || '(未填)',   // 發生單位 → 發生地
      productId:  '',                    // LIFF 沒有產品編號欄位，留空
      itemName:   d.product || '',       // 品名
      issue:      `【${d.anomaly}】 比例：${d.ratio} 責任：${d.resp} 判定：${d.judge}`,
      vendor:     d.resp || '',          // 責任單位 → 異常廠商
      customer:   '',
      solution:   d.judge || '',         // 品質判定 → 處理方式
      quantity:   d.qty || '',           // 訂單數量
      caseNumber: '',
      skipReason: '',
      photoUrl:   d.photoData || null,
    };

    const page = await createNotionPage(notionData, d.userName || 'LIFF');

    // 推播通知給主管
    const judgeEmoji = d.judge === '驗退' ? '❌' : d.judge === '特採' ? '⚠️' : '🔧';
    const msg =
      `【LIFF 異常通報】\n` +
      `📦 品名：${d.product}\n` +
      `📍 發生單位：${d.unit}\n` +
